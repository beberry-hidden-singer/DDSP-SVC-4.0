"""modified from https://github.com/yxlllc/DDSP-SVC/blob/master/preprocess.py
by karljeon44

no speaker encoder => then practically the same as Diffusion-SVC's preprocess.py
"""
import argparse
import logging
...
def preprocess(path, f0_extractor, volume_extractor, mel_extractor, units_encoder, sample_rate, hop_size,
               device='cuda', use_pitch_aug=False,extensions=['wav']):
...
if __name__ == '__main__':
  ...
  volume_extractor = VolumeExtractor(args.data.block_size)
  ...
  preprocess(args.data.train_path, f0_extractor, volume_extractor, mel_extractor, units_encoder, sample_rate, hop_size,
             device=device, use_pitch_aug=use_pitch_aug, extensions=extensions)

  # preprocess validation set
  preprocess(args.data.valid_path, f0_extractor, volume_extractor, mel_extractor, units_encoder, sample_rate, hop_size,
             device=device, use_pitch_aug=False, extensions=extensions)
