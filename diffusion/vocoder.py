...
from ddsp_naive.naive import Unit2MelNaive
from ddsp_naive.vocoder import CombSubFast
from diffusion.diffusion import GaussianDiffusion
from diffusion.wavenet import WaveNet
from nsf_hifigan.models import load_config, load_model
...
def load_model_vocoder(model_path, device='cpu', loaded_vocoder=None):
  ...
  model = load_svc_model(args, vocoder.dimension)
...
class Unit2Wav(nn.Module):
  ...
  def forward(self, units, f0, volume, spk_id=None, spk_mix_dict=None, aug_shift=None, vocoder=None,
    ...
    return:
        dict of B x n_frames x feat
    '''
    ddsp_wav, hidden, (_, _) = self.ddsp_model(
      units, f0, volume, spk_id=spk_id, spk_mix_dict=spk_mix_dict, aug_shift=aug_shift, infer=infer)
