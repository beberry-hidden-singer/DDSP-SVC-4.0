...
class VolumeExtractor:
  def __init__(self, hop_size = 512, block_size=None):
    self.hop_size = hop_size
  ...
  def get_mask_from_volume(self, volume, block_size=None, threhold=-60.0, device='cpu'):
...
class UnitsEncoder:
  def __init__(self, encoder, encoder_ckpt, encoder_sample_rate = 16000, encoder_hop_size = 320, device = None,
    ...
    if encoder == 'contentvec768l12':
      ...
      is_loaded_encoder = True
    if encoder in ['hubertkor', 'hubertbasekor', 'hubertbasekorean']:
      self.model = HubertBaseKorean(encoder_ckpt, device=device)
      is_loaded_encoder = True
...
class HubertBaseKorean:
  def __init__(self, path, h_sample_rate=16000, h_hop_size=320, device='cpu'):
    self.device = device
    print(' [Encoder Model] Hubert-Base Korean')
    self.hubert = HubertModel.from_pretrained("team-lucid/hubert-base-korean").to(device)
    self.hubert.eval()

  def __call__(self, audio):  # B, T
    # wav_tensor = torch.from_numpy(audio).to(self.device)
    wav_tensor = audio
    feats = wav_tensor.view(1, -1).to(wav_tensor.device)
    with torch.no_grad():
...
def load_model(model_path, device='cpu'):
  ...
  assert args.model.type == 'CombSubFast'
  model = CombSubFast(
    sampling_rate=args.data.sampling_rate,
    block_size=args.data.block_size,
    n_unit=args.data.encoder_out_channels,
